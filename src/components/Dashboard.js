...
import 'react-toastify/dist/ReactToastify.css'; // Make sure this is at the top
...
const DashboardHome = () => {
  ...
  const handleSwitchToggle = async (deviceId, switchIndex) => {
    try {
      ...
      setEsp32Devices(prevDevices =>
        prevDevices.map(device => {
          if (device.id === deviceId) {
            const updatedSwitches = [...device.switches];
            updatedSwitches[switchIndex] = !updatedSwitches[switchIndex];
            return {
              ...device,
      ...
      toast.success('Switch state updated successfully');
    } catch (error) {
  ...
  const handleEditSwitchLabel = (deviceId, switchIndex, newLabel) => {
    setEsp32Devices(prevDevices =>
      prevDevices.map(device => {
        if (device.id === deviceId) {
          const updatedLabels = [...device.switchLabels];
          updatedLabels[switchIndex] = newLabel;
          return { ...device, switchLabels: updatedLabels };
        }
        return device;
      })
    );
  ...
  const handleDeleteDevice = async (deviceId) => {
    ...
    try {
      ...
      setEsp32Devices(prev => prev.filter(device => device.id !== deviceId));
